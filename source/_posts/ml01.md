...
7. Life-long Learning
    - 終身學習，同樣叫做 Continuous Learning, Never Ending Learning, Incremental Learning
    - 讓機器終身學習、不斷學習下去
    - 學好一個東西就繼續學下一個
...
1. $w$ 對 $L$ 的偏微分：
    - $\dfrac{\partial L}{\partial w} \vert_{w=w^0,b=b^0} = \sum{2(\hat{y}^n - (b + w * x_{cp}^n))*(-x_{cp}^n)}$
2. $b$ 對 $L$ 的偏微分：
    - $\dfrac{\partial L}{\partial b}\vert_{w=w^0,b=b^0} = \sum{2(\hat{y}^n - (b + w * x_{cp}^n))*(-1)}$
